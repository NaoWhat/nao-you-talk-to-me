...
class NaoTorsoController():
    ...
    def moveTorso(self, dx, dy, dz):
        # Example showing how to use positionInterpolations
        frame = motion.FRAME_ROBOT
        useSensorValues = False

        # Motion of Torso with block process
        ...
        torsoList  = []
        currentPos = self.motion_service.getPosition("Torso", frame, useSensorValues)
        ...
        if targetPos.y > -0.04 and targetPos.y < 0.04:
...
if __name__ == "__main__":
   torso_controller = NaoTorsoController('lapis.local')

   for i in range(0, 3):
        torso_controller.moveTorso(0, -0.005, 0)
